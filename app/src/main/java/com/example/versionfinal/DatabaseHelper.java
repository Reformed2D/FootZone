...
public static final String TABLE_NAME = "users";
    ...
    public static final String COL_12 = "IS_VERIFIED";
/*******************************************************************/
...
public DatabaseHelper(Context context) {
    ...
    ensureVerificationColumnExists();
}
    @Override
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 13) {
            db.execSQL("ALTER TABLE " + TABLE_NAME + " ADD COLUMN " + COL_12 + " INTEGER DEFAULT 0");// Adjust as needed.
    ...
    public boolean updateVerificationStatus(String email, boolean isVerified) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(COL_12, isVerified ? 1 : 0);
        return db.update(TABLE_NAME, values, COL_3 + "=?", new String[]{email}) > 0;
    }
    ...
    private void ensureVerificationColumnExists() {
        SQLiteDatabase db = this.getWritableDatabase();
        ...
        if (!columnExists) {
            try {
                ...
                Log.e("DatabaseHelper", "Error adding IS_VERIFIED column: " + e.getMessage());
            }
        }
    }
